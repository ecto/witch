...
val* builtin_sub(env* e, val* a) { return builtin_op(e, a, "-"); }
val* builtin_mul(env* e, val* a) { return builtin_op(e, a, "*"); }
val* builtin_div(env* e, val* a) { return builtin_op(e, a, "/"); }
...
val* builtin_var(env* e, val* a, char* func) {
  ...
  for (int i = 0; i < syms->count; i++) {
    if (strcmp(func, "def") == 0) { env_def(e, syms->cell[i], a->cell[i+1]); }
    if (strcmp(func, "=")   == 0) { env_put(e, syms->cell[i], a->cell[i+1]); } 
  }
...
val* builtin_put(env* e, val* a) { return builtin_var(e, a, "="); }
...
val* builtin_gt(env* e, val* a) { return builtin_ord(e, a, ">");  }
val* builtin_lt(env* e, val* a) { return builtin_ord(e, a, "<");  }
val* builtin_ge(env* e, val* a) { return builtin_ord(e, a, ">="); }
val* builtin_le(env* e, val* a) { return builtin_ord(e, a, "<="); }

val* builtin_cmp(env* e, val* a, char* op) {
  ASSERT_NUM(op, a, 2);
  int r;
  if (strcmp(op, "==") == 0) { r =  val_eq(a->cell[0], a->cell[1]); }
  if (strcmp(op, "!=") == 0) { r = !val_eq(a->cell[0], a->cell[1]); }
  val_del(a);
  return val_num(a->state, r);
}

val* builtin_eq(env* e, val* a) { return builtin_cmp(e, a, "=="); }
val* builtin_ne(env* e, val* a) { return builtin_cmp(e, a, "!="); }
